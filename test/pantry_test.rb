...
class PantryTest < Minitest::Test
  ...
  def test_it_can_provide_recommendations_based_on_ingredients_in_stock
    ...
    assert_equal true, pantry.check_if_has_single_ingredient("Raw nuts", 10)
    assert_equal false, pantry.check_if_has_single_ingredient("Cheese", 20)
    assert_equal false, pantry.check_each_ingredient(r1)
    assert_equal true, pantry.check_each_ingredient(r2)
    assert_equal [r2,r3], pantry.recipes_can_make
    assert_equal ["Pickles", "Peanuts"], pantry.what_can_i_make
  end

  def test_it_can_determine_how_many_i_can_make
    pantry = Pantry.new
    ...
    assert_equal 2, pantry.amount_per_ingredient("Salt", 10)
    assert_equal 0, pantry.amount_per_ingredient("Cheese", 20)
    assert_equal 0, pantry.amount_per_recipe(r1)
    assert_equal 4, pantry.amount_per_recipe(r2)
